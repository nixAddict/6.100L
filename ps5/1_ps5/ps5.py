...
def filter(pixels_list, color):
    ...
    for pixel in pixels_list:
        filtered_pixel = [round(i) for i in matrix_multiply(transformation_matrix, pixel)]
        filtered_pixels_list.append(tuple(filtered_pixel))
...
def reveal_bw_image(filename):
    ...
    new_pixels = [i * 255 for i in new_pixels]
    with Image.open(filename) as im:
